...
async function apply(ctx, config = {}) {
    ...
    // No portable default exists, so it must be set explicitly via
    // DSH_MINIMAX_SKILLS_ROOT; when unset, get_skill/list_skills report a
    // clear error instead of scanning a developer-machine path.
    ...
    register(strDef({
      ...
      execute: async (args, exec) => {
        try {
          ...
          if (skills === null) return '(skills root not configured; set DSH_MINIMAX_SKILLS_ROOT to the mini-agent skills directory)'
    ...
    register(strDef({
      ...
      execute: async (args, exec) => {
        try {
          const skills = await discoverSkills(exec.signal)
          if (skills === null) return 'get_skill error: skills root not configured; set DSH_MINIMAX_SKILLS_ROOT to the mini-agent skills directory.'
